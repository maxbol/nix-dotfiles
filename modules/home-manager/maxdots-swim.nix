...
with lib; let
  ...
  wallpaperDir =
    ...
    then "${config.maxdots.chroma.themeDirectory}/active/swim/wallpapers"
...
in {
  options = {
    ...
    maxdots.chroma.swim.enable =
      mkEnableOption "Chroma integration for Swim"
      // {
        readonly = true;
        default = config.maxdots.swim.chromaIntegration.enable;
      };
  };
  ...
  config = mkIf cfg.enable {
    ...
    maxdots.chroma.programs.swim = mkIf cfg.chromaIntegration.enable {
      themeOptions = {
        wallpaperDirectory = mkOption {
          type = with types; nullOr (oneOf [str path]);
