...
in {
  options = {
    maxdots.chroma.hyprland.enable = mkOption {
      type = types.bool;
  ...
  config = {
    assertions = [
      {
        assertion = !(cfg.enable && config.maxdots.chroma.hyprland.enable) || config.wayland.windowManager.hyprland.enable;
        message = "Chroma's Hyprland integration only works when the Hyprland module is enabled";
      }
      {
        assertion = !(cfg.enable && config.maxdots.chroma.hyprland.enable) || config.maxdots.chroma.desktop.enable;
        message = "Chroma's desktop module is required for the Hyprland module.";
      }
    ];

    maxdots.chroma.programs.hyprland = {
      activationCommand = {
        name,
        opts,
      }: let
    ...
    wayland.windowManager.hyprland = mkIf (cfg.enable && config.maxdots.chroma.hyprland.enable) {
      settings = {
    ...
    home.packages =
      mkIf (cfg.enable && config.maxdots.chroma.hyprland.enable)
