...
in {
  options = {
    maxdots.chroma.qt.enable = mkOption {
      type = types.bool;
      default = false;
      example = false;
      description = ''
  ...
  imports = [
    (mkIf (cfg.enable && cfg.qt.enable) {
      ...
      xdg.configFile."qt5ct/qt5ct.conf".source = config.lib.file.mkOutOfStoreSymlink "${config.maxdots.chroma.themeDirectory}/active/qt/qt5ct.conf";
      xdg.configFile."qt6ct/qt6ct.conf".source = config.lib.file.mkOutOfStoreSymlink "${config.maxdots.chroma.themeDirectory}/active/qt/qt5ct.conf";
