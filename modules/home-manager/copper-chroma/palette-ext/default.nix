...
in {
  options = {
    copper.chroma.palette-ext.enable = mkOption {
      type = types.bool;
      default = true;
      readOnly = true;
      description = ''
  ...
  config = mkMerge [
    {
      copper.chroma.programs.palette-ext = {
        themeOptions = {
        ...
        themeConfig = {config, ...}: {
          generateDynamic = {
            ...
            with lib; let
              ...
              palette = config.file."palette-ext.json".source;
          ...
          file."palette-ext.json".text = builtins.toJSON {inherit (config) semantic colors accents;};
        };
      };
    }
  ];
